...
results <- read.csv(paste0(getwd(),"/SEFS Graduate Student Finances Survey_4.25.csv"))
...
for (i in 1:length(pieQs)){
  ...
  sumQ <- sum(qResults$n)
  jpeg(file=paste0("Q-",names(results[pieQs[i]]),".jpg"),width=812, height=502)
  pie(qResults$n,
           ...
           main = gsub('\\.',' ',names(results[pieQs[i]])))
  dev.off()
}
